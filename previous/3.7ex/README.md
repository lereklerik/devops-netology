...
Учитывая, что в 1 задании пропускная способность, при самой маленькой вероятности потери пакетов, была рассчитана в **389,33 МБит/с**, то при 1% потере пакетов
скорость упала примерно в **1000** раз до **0,39 МБит/с** (если быть точным, то предыдущее вычисление больше полученного в **998,2** раз). 
...
####  Определение маршрута с участием протокола **ARP**:
    ...
    *    Если адреса находятся в одной подсети, то вызывается протокол ARP и определяется физический адрес получателя, после чего IP-пакет инкапсулируется во фрейм канального уровня и отправляется по указанному физическому адресу, соответствующему IP-адресу назначения.
    *    Если нет – начинается просмотр таблицы в поисках прямого маршрута.
    *    Если маршрут найден, то вызывается протокол ARP и определяется физический адрес соответствующего маршрутизатора, после чего пакет инкапсулируется во фрейм канального уровня и отправляется по указанному физическому адресу.
    *    В противном случае, вызывается протокол ARP и определяется физический адрес маршрутизатора по умолчанию, после чего пакет инкапсулируется во фрейм канального уровня и отправляется по указанному физическому адресу.
